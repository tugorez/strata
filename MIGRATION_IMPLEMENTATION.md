...
### 1. **Migration Generator** (`strata_builder`)
- **File**: `packages/strata_builder/lib/src/migrations_generator.dart`
- **Purpose**: Scans `migrations/` directory for `.sql` files and generates a single Dart file
- **Features**:
  - Automatic discovery of migration files
  - Version number extraction from filenames
  - SQL statement parsing
  - Generates `lib/migrations.g.dart` with all migrations
...
- **File**: `packages/strata_builder/build.yaml`
- **Added**: `migrationsBuilder` configuration
- **Features**:
  - Auto-applies to dependent packages
  - Runs before schema builder
  - Generates to source directory
...
packages/
  ...
  strata_sqlite/                    # SQL-Specific
    lib/
      src/
        sql_migration.dart          # NEW - SqlMigration data class
        sql_migration_runner.dart   # SQL migration execution
        sqlite_adapter.dart         # SQLite implementation
  
  strata_builder/                   # Code Generation (SQL-focused)
    lib/
      src/
        migrations_generator.dart   # Generates SqlMigration from .sql files
    bin/
...
```dart
// lib/migrations.g.dart (auto-generated)
import 'package:strata_sqlite/strata_sqlite.dart';
